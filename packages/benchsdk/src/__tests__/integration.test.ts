...
describeIntegration('benchmark orchestrator integration', () => {
  it('smokes the live platform orchestrator API contract', async () => {
    ...
    try {
      ...
      expect(['ready', 'complete', 'partial', 'pending', 'unavailable', 'failed', 'importing']).toContain(overviewResults.analytics.status);
      ...
      for (const item of overviewResults.items) {
        expect(['ready', 'complete', 'partial', 'pending', 'unavailable', 'failed', 'importing']).toContain(item.analytics.status);
      }
