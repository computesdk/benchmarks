...
import { resolveCopilotHost } from './github-copilot-host.js';
...
export const modelIndexProviders: AIGatewayModelIndexProviderConfig[] = [
  ...
  },
  {
    ...
    buildHeaders: () => ({
      ...
      'User-Agent': 'computesdk-benchmarks',
      Authorization: `Bearer ${process.env.GITHUB_COPILOT_OAUTH_TOKEN ?? process.env.GITHUB_COPILOT_API_KEY}`,
    }),
    resolveHost: async (headers, log) => {
      const authHeader = headers.Authorization ?? headers.authorization;
      const token =
        typeof authHeader === 'string'
          ? authHeader.replace(/^Bearer\s+/i, '')
          : (process.env.GITHUB_COPILOT_OAUTH_TOKEN ?? process.env.GITHUB_COPILOT_API_KEY);
      return token ? resolveCopilotHost(token, log) : undefined;
    },
  },
];
