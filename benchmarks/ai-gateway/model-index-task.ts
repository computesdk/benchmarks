...
import { maskSecrets } from './github-copilot-host.js';
import type {
...
function fetchJson(
  ...
  log?.(`${prefix}GET https://${host}${requestPath}`);
    ...
    const req = https.request(
      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          const masked = maskSecrets(body.slice(0, 500));
          if (!res.statusCode || res.statusCode < 200 || res.statusCode >= 300) {
            log?.(`${prefix}HTTP ${res.statusCode ?? 'unknown'}: ${masked}`);
            settle({ statusCode: res.statusCode, responseMs, error: `HTTP ${res.statusCode}: ${masked}` });
            return;
          }
          log?.(`${prefix}HTTP ${res.statusCode} (${responseMs}ms): ${masked.slice(0, 200)}`);
...
async function fetchModelList(
  ...
  if (config.resolveHost) {
    const resolved = await config.resolveHost(headers, log);
    if (!resolved) {
      return {
        responseMs: Date.now() - startedAt,
        error: 'Host resolution failed; see logs for details',
      };
    }
    host = resolved.host;
...
export async function runModelIndexTask(
  ...
  const startedAt = Date.now();
  const log = (message: string) => ctx.log(message);

  const { statusCode, body, responseMs, error } = await fetchModelList(config, log);
  ...
  if (error) {
    ...
    return { data: result as unknown as JsonObject, latencyMs: Date.now() - startedAt };
  }
